...
import { deleteLeaveMessage, isCheckLeaveMessage, leaveMessageList, searchLeaveMessage } from '~/api/blog/leave-word'
...
async function refreshFunc() {
  loading.value = true
  const { data } = await leaveMessageList()
...
async function onFinish(values: any) {
  ...
  const { data } = await searchLeaveMessage(submitData)
  if (data && data.length > 0) {
    tabData.value = data.map((item: any) => {
      item.isCheck = item.isCheck === 1
      return item
...
function updateIsCheckFunc(id: string, isCheck: any, record: any) {
  ...
  isCheck = isCheck ? 1 : 0
  isCheckLeaveMessage({ id, isCheck }).then((res) => {
    if (res.code === 200) {
...
function onDelete(ids?: string[]) {
  if (ids) {
    deleteLeaveMessage(ids).then((res) => {
      if (res.code === 200) {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk: () => {
        deleteLeaveMessage(ids as string[]).then((res) => {
          if (res.code === 200) {
            message.success('删除成功')
            refreshFunc()
          }
  ...
  >
    ...
    <template #operate-btn>
      ...
      <a-modal v-model:open="contentModel.show" width="700px">
        <template #title>
          <span style="color: #1677FF"><MessageOutlined /></span><span style="margin-left: 0.2rem">查看留言</span>
        </template>
        <template #footer>
