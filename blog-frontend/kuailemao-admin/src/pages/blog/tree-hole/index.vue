...
async function onFinish(values: any) {
  ...
  const { data } = await searchTreeHole(submitData)
  if (data && data.length > 0) {
    tabData.value = data.map((item: any) => {
      item.isCheck = item.isCheck === 1
      return item
...
function updateIsCheckFunc(id: string, isCheck: any, record: any) {
  ...
  isCheck = isCheck ? 1 : 0
  isCheckTreeHole({ id, isCheck }).then((res) => {
    if (res.code === 200) {
...
function onDelete(ids?: string[]) {
  if (ids) {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk: () => {
        deleteTreeHole(ids as string[]).then((res) => {
          if (res.code === 200) {
            message.success('删除成功')
            refreshFunc()
          }
