...
import { addRolePermission, queryPermissionNotRole } from '~/api/permission/role-permission'
...
async function refreshFunc(searchData?: { roleName: string | undefined; roleKey: string | undefined }) {
  ...
  let newData: any = []
  if (searchData) {
    const { data } = await queryPermissionNotRole(route.params.id as string, searchData.roleName, searchData.roleKey)
    newData = data
  }
  else {
    const { data } = await queryPermissionNotRole(route.params.id as string)
    if (data)
      newData = data
  }
...
async function handleOk() {
  ...
  if (state.selectedRowKeys.length > 0 && route.params.id) {
    ...
    const data = await addRolePermission(addData)
    if (data.code === 200)
  ...
  <a-modal v-model:open="modalOpen" width="50%" :title="title" @ok="handleOk" @cancel="openBtnFunc">
    ...
    >
      <div style="display: flex">
        <a-form-item label="角色名称" name="roleKey" style="margin-right: 1rem">
          <a-input v-model:value="searchData.roleKey" placeholder="请输入角色名称" />
        </a-form-item>
        <a-form-item label="角色字符" name="roleName" style="margin-right: 2rem">
          <a-input v-model:value="searchData.roleName" placeholder="请输入角色字符" style="width: 220px" />
        </a-form-item>
        <a-form-item>
