...
import { deleteRolePermission, queryPermissionRole } from '~/api/permission/role-permission'
...
async function refreshFunc(searchData?: object) {
  ...
  if (searchData) {
    ...
    const { data } = await queryPermissionRole(route.params.id as string)
    if (data)
      newData = data
  }
...
async function onFinish(values: { roleName: string; roleKey: string }) {
  loading.value = true
  const { data } = await queryPermissionRole(route.params.id as string, values.roleName, values.roleKey)
...
async function deleteFunc(roleId: string[]) {
  ...
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk: async () => {
      const data = await deleteRolePermission(deleteData)
      if (data.code === 200) {
